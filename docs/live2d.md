...
窗口默认由 manifest 决定，最大 640×900 逻辑像素并限制在工作区。Live2D 按当前可见部件的三角网格维护一张 64×96 的命中遮罩（768 字节），大约每 200ms 重算一次，内容不变则不发送。复用三角形缓冲，跳过屏幕外和已经覆盖的区域；隐藏时随渲染暂停，缩放或位置改变时立即更新。旧版本地 runtime 没有三角网格接口时兼容回退到部件包围盒；更新本地 runtime 后启用新路径。光标落在空白格时，形象窗口带上 `WS_EX_LAYERED` 并打开 `WS_EX_TRANSPARENT`，点击落到下面的窗口；光标回到人物上时关掉透明命中位，分层样式保留，人物画面保持不变。判断大约每 50ms 读一次光标位置，并且只在命中结果变化时改窗口样式。不读取像素、不用全局鼠标 hook、也不子类化窗口。网格边缘外扩约 3 个逻辑像素，最大窗口的格子约 10 像素；比部件矩形更贴合轮廓，但不解析贴图透明度或裁剪蒙版，因此不是逐像素穿透。静态图和视频形象仍然占用整个窗口。

## 验证

2026-09-22 穿透遮罩优化：本机 Edge 真实 Haru/Ren 模型，按最大 640×900 窗口连续采样 30 次，三角网格提取与遮罩生成中位耗时分别约 2ms/3ms，P95 约 3.5ms/3.9ms。5Hz 下折算为每秒约 10–15ms 的计算时间，仅为此段算法的短时测量，不代表应用总 CPU/GPU 占用。58 项形象单元测试、3 项 Rust 穿透测试、类型检查、lint、前端构建和 22 次真实模型切换通过。实际桌面下层窗口点击、快速越过边缘与多 DPI 拖动仍需人工验收。
